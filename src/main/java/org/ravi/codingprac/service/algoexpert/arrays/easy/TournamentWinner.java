...
/*
    * Tournament Winner
    * There's an algorithm's tournament taking place in which teams of programmers compete against each other
    * to solve algorithmic problems as fast as possible. Teams compete in a round robin, where each team faces off against
    ...
    * and results, respectively. The competitions array has elements in the form of [homeTeam, awayTeam],
    * where each team is a string of at most 30 characters representing the name of the team. The results array contains
    * information about the winner of each corresponding competition in the competitions array. Specifically, results[i] denotes
    * the winner of competitions[i], where a 1 in the results array means that the home team in the corresponding competition
    ...
    *
    * Hint3:
    * Loop through all of the competitions, and update the hash table at every iteration. For each competition, consider the name of
    * the winning team; if the name already exists in the hash table, update that entry by adding 3 points to it. If the team name doesn't
    * exist in the hash table, add a new entry in the hash table with the key as the team name and the value as 3 (since the team won its
    * first competition). While looping through all of the competitions, keep track of the team with the highest score, and at the end of the
...
public interface TournamentWinner {
    ...
    public String winnerOfTheTournament2(ArrayList<ArrayList<String>> competitions, ArrayList<Integer> results);
    public String winnerOfTheTournament3(ArrayList<ArrayList<String>> competitions, ArrayList<Integer> results);
}
